...
var textpattern = (function () {
  ...
  var $_djl07yqjjfh9qdbx = { getPatterns: getPatterns };
  ...
  var patternFromRng = function (patterns, rng, space) {
    ...
    var endPattern = $_84kebgqrjfh9qdca.findEndPattern(patterns, text, rng.startOffset, delta);
    ...
    var startPattern = $_84kebgqrjfh9qdca.findPattern(patterns, patternRng.toString());
  ...
  var applyBlockFormat = function (editor, patterns) {
    ...
    if (textBlockElm) {
      ...
      if (firstTextNode) {
        pattern = $_84kebgqrjfh9qdca.findPattern(patterns, firstTextNode.data);
        if (!pattern) {
          return;
        }
  ...
  function handleEnter(editor, patterns) {
    ...
    wrappedTextNode = $_9oj7icqojfh9qdc6.applyInlineFormatEnter(editor, patterns);
    if (wrappedTextNode) {
    ...
    $_9oj7icqojfh9qdc6.applyBlockFormat(editor, patterns);
  }
  function handleInlineKey(editor, patterns) {
    var wrappedTextNode, lastChar, lastCharNode, rng, dom;
    wrappedTextNode = $_9oj7icqojfh9qdc6.applyInlineFormatSpace(editor, patterns);
    if (wrappedTextNode) {
      dom = editor.dom;
  ...
  var setup = function (editor, patternsState) {
    ...
    editor.on('keydown', function (e) {
      if (e.keyCode === 13 && !global$2.modifierPressed(e)) {
        $_ei7b7wqnjfh9qdc2.handleEnter(editor, patternsState.get());
      }
    }, true);
    editor.on('keyup', function (e) {
      if ($_ei7b7wqnjfh9qdc2.checkKeyCode(keyCodes, e)) {
        $_ei7b7wqnjfh9qdc2.handleInlineKey(editor, patternsState.get());
      }
    });
    editor.on('keypress', function (e) {
      if ($_ei7b7wqnjfh9qdc2.checkCharCode(charCodes, e)) {
  ...
  var $_aiwetpqkjfh9qdbz = { setup: setup };

  global.add('textpattern', function (editor) {
    var patternsState = Cell($_djl07yqjjfh9qdbx.getPatterns(editor.settings));
    $_aiwetpqkjfh9qdbz.setup(editor, patternsState);
    return $_4ixznpqijfh9qdbu.get(patternsState);
  });
  function Plugin () {
  }
