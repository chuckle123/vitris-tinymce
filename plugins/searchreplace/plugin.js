...
var searchreplace = (function () {
  ...
  var $_8is6cwiujfh9qbnw = { findAndReplaceDOMText: findAndReplaceDOMText };
  ...
  var markAllMatches = function (editor, currentIndexState, regex) {
    ...
    return $_8is6cwiujfh9qbnw.findAndReplaceDOMText(regex, node, marker, false, editor.schema);
  };
  ...
  var get = function (editor, currentIndexState) {
    var done = function (keepEditorSelection) {
      return $_9gt9kdisjfh9qbno.done(editor, currentIndexState, keepEditorSelection);
    };
    var find = function (text, matchCase, wholeWord) {
      return $_9gt9kdisjfh9qbno.find(editor, currentIndexState, text, matchCase, wholeWord);
    };
    var next = function () {
      return $_9gt9kdisjfh9qbno.next(editor, currentIndexState);
    };
    var prev = function () {
      return $_9gt9kdisjfh9qbno.prev(editor, currentIndexState);
    };
    var replace = function (text, forward, all) {
      return $_9gt9kdisjfh9qbno.replace(editor, currentIndexState, text, forward, all);
    };
    return {
      done: done,
      find: find,
  ...
  var open = function (editor, currentIndexState) {
    ...
    function updateButtonStates() {
      win.statusbar.find('#next').disabled($_9gt9kdisjfh9qbno.hasNext(editor, currentIndexState) === false);
      win.statusbar.find('#prev').disabled($_9gt9kdisjfh9qbno.hasPrev(editor, currentIndexState) === false);
    }
    ...
    var win = editor.windowManager.open({
      ...
      onClose: function () {
        editor.focus();
        $_9gt9kdisjfh9qbno.done(editor, currentIndexState);
        editor.undoManager.add();
      },
      onSubmit: function (e) {
        ...
        if (!text.length) {
          $_9gt9kdisjfh9qbno.done(editor, currentIndexState, false);
          win.statusbar.items().slice(1).disabled(true);
          return;
        }
        if (last.text === text && last.caseState === caseState && last.wholeWord === wholeWord) {
          if (!$_9gt9kdisjfh9qbno.hasNext(editor, currentIndexState)) {
            notFoundAlert();
            return;
          }
          $_9gt9kdisjfh9qbno.next(editor, currentIndexState);
          updateButtonStates();
          return;
        }
        count = $_9gt9kdisjfh9qbno.find(editor, currentIndexState, text, caseState, wholeWord);
        if (!count) {
          notFoundAlert();
        }
      ...
      buttons: [
        ...
        },
        {
          ...
          disabled: true,
          onclick: function () {
            if (!$_9gt9kdisjfh9qbno.replace(editor, currentIndexState, win.find('#replace').value())) {
              win.statusbar.items().slice(1).disabled(true);
              currentIndexState.set(-1);
              last = {};
            }
        ...
        },
        {
          ...
          disabled: true,
          onclick: function () {
            $_9gt9kdisjfh9qbno.replace(editor, currentIndexState, win.find('#replace').value(), true, true);
            win.statusbar.items().slice(1).disabled(true);
            last = {};
          }
        },
        ...
        },
        {
          ...
          disabled: true,
          onclick: function () {
            $_9gt9kdisjfh9qbno.prev(editor, currentIndexState);
            updateButtonStates();
          }
        },
        {
  ...
  var $_vg5mxiwjfh9qboa = { open: open };

  var register = function (editor, currentIndexState) {
    editor.addCommand('SearchReplace', function () {
      $_vg5mxiwjfh9qboa.open(editor, currentIndexState);
    });
  };
  var $_apidqnivjfh9qbo8 = { register: register };
  ...
  var $_1jzd0gixjfh9qbod = { register: register$1 };

  global.add('searchreplace', function (editor) {
    var currentIndexState = Cell(-1);
    $_apidqnivjfh9qbo8.register(editor, currentIndexState);
    $_1jzd0gixjfh9qbod.register(editor, currentIndexState);
    return $_zlzevirjfh9qbnk.get(editor, currentIndexState);
  });
  function Plugin () {
  }
